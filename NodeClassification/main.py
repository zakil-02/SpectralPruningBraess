...
parser.add_argument('--model', type=str, default='GCN', choices=['GCN', 'GATv2'], help='Model to use')
...
print(f'node label informativeness: {nodeliaf:.4f}')
...
gcn_time = (end_gcn - start_gcn)
print()  
headers = ['Method','Dataset','AvgTestAcc', 'Deviation','InitialGap','EdgesModified','UpdatePeriod','FinalGap',
          'NLIAfter','AdjHomAfter','NumLayers','Model','LR','Dropout','HiddenDimension','AvgTrainingAcc','Deviation', 
          'ModifyingTime','TrainingTime']

with open(filename, mode='a', newline='') as file:
              writer = csv.writer(file)
              if file.tell() == 0:
                      writer.writerow(headers)
              writer.writerow([args.method,args.dataset,np.mean(avg_acc_testallsplits), np.std(avg_acc_testallsplits),
              initialgap,max_iterations*update_period,update_period,fgap,nodeliaf,hadjaf,args.num_layers,args.model,
              lr,p,hidden_channel,np.mean(trainallsplits),np.std(trainallsplits),
              data_modifying,gcn_time])
